...
function VerifyControls({ studentId, document }: { studentId: number; document: DocumentRow }) {
    ...
    function submit(status: 'verified' | 'rejected') {
        setProcessing(true);
        router.patch(
            route('students.documents.verify', [studentId, document.id]),
...
export default function Documents({
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(route('students.documents.store', studentId), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                reset();
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {documents.map((doc) => (
                                <tr key={doc.id}>
                                    <td className="px-5 py-2.5 capitalize">{doc.category.replace(/_/g, ' ')}</td>
                                    <td className="px-5 py-2.5">{doc.title}</td>
                                    <td className="px-5 py-2.5">
                                        <a
                                            href={route('students.documents.download', [studentId, doc.id])}
                                    ...
                                    {canManage && (
                                        <td className="px-5 py-2.5 text-right">
                                            <SecondaryButton
                                                ...
                                                onClick={() =>
                                                    router.delete(
                                                        route('students.documents.destroy', [studentId, doc.id]),
                                                        { preserveScroll: true },
                                                    )
                                                }
                                            >
