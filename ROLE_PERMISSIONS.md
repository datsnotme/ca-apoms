...
`DeviceController`, `/sync/*`, live since Phase 4, device management since the Phase 6 follow-up),
and both `auth:sanctum` (device token identity) *and* `permission:sync.manage` together gate
`routes/api.php`'s sync endpoints, live since Phase 2. A device token is issued to a specific `User`
— via `sync:register-device`, or now the Devices page's register form, which only offers users who
already have `sync.manage` — so it only ever has that user's own permissions — a token can never
grant more access than the person it represents already has through the normal web app.
