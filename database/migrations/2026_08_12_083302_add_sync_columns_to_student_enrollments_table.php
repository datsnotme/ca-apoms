...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::table('student_enrollments')->whereNull('uuid')->orderBy('id')->chunkById(500, function ($rows) {
            foreach ($rows as $row) {
                DB::table('student_enrollments')->where('id', $row->id)->update(['uuid' => (string) Str::uuid()]);
            }
        });
    }
